...
export default function EditJobseeker() {
  ...
  useEffect(() => {
    const fetchJobseeker = async () => {
      try {
        const res = await fetch(`/api/jobseekers/${id}`);
        if (res.ok) {
          const data = await res.json();
          reset(data);
        } else {
          toast.error('Jobseeker not found');
          router.push('/jobseekers');
        }
      } catch (error) {
        console.error('Error fetching jobseeker:', error);
        toast.error('Error fetching jobseeker');
        router.push('/jobseekers');
      }
    };
    fetchJobseeker();
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error(error.error || 'Failed to update jobseeker');
      }
    } catch (error) {
      console.error('Error updating jobseeker:', error);
      toast.error('Error updating jobseeker');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 text-gray-800">
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Edit Jobseeker</h1>
            <p className="mt-1 text-gray-600">Update jobseeker details below</p>
          </div>
          <a
            href="/jobseekers"
